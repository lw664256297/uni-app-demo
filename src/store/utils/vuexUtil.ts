...
export function decorator<D extends BindingHelper, T>(helper: D, keyMap: T) {
  ...
  type Decorator = (target: any, key: string) => void;

  function decoratorWrapper(target: any, key: KeyType): void;
  function decoratorWrapper(originKey: KeyType): Decorator;
  function decoratorWrapper(a: any, b?: KeyType): Decorator | void {
    if (typeof b === "string") {
      const target = a;
      const key = b;
